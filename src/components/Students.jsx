...
import { allStudentApi } from '../slice/studentSlice'
...
const Students = () => {
  ...
  //  modal for view, edit, delete button
  ...
  const handleDelete = async () => {
    const { data } = await axios.delete(`${import.meta.env.VITE_API_URL}/student/student-edit/${selectedata}`, getConfig());
    if (data.status === 200) {
  ...
  // getting the student list data Statically ends.
  // async function connect() {
  // const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/student/all-student`, data, getConfig()); 
  //   console.log(data);
  //   setStudent(data);
  // };
  ...
  //  to get all the students list from the database dynamically
  ...
  // async function connect() {
  //   const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/student/get-all-student`, getConfig());
  //   setStudent(data);
  // }
  ...
  useEffect(() => {
    if (studentStore.students?.length > 0) {
      ...
      setStudent(studentStore.students);
    }
          ...
          <div className="col-lg-12 text-center">
            <div className="card p-3">
              <div className='table_wrapper'>
                <table className='table cutom_table' >
                  <thead className=''>
...

                    {
                      student.length > 0 ?
                        student.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td>
                              {
                                item.isEnrolled === true ?
                                  <button type='button' className='btn badge btn-success btn-sm '>In-Active</button>
                                  :
                            ...
                            <td className='justify-content-center'>
                              <div className='d-flex'>
                                ...
                                <button onClick={() => { setShowViewModel(true) }} type='button' data-bs-toggle="modal" data-bs-target="#exampleModal" className='  btn  btn-sm btn-outline-warning me-2'>
                                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-pencil-square" viewBox="0 0 16 16">
                                    <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                    <path fillRule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z" />
                                  </svg>
                                </button>
                                <button onClick={() => { setShowDeleteModel(true); setSelectedData(item._id); }} type='button' className='btn  btn-sm btn-outline-danger me-2'>
                          ...
                          <tr>
                            <td colSpan={9} className='text-center'> No Students in record    </td>
                          </tr>
                        )
                    }
                ...
                <div className="modal d-block" tabIndex="-1">
                  <div className="modal-dialog">
                    <div className="modal-content">
                      ...
                      <div className="modal-body">
                        <p>Are you sure you want to  <span className=''>delete this book?</span></p>
                      </div>

                      <div className="modal-footer">
                        <button ref={closeBtnRef} className="btn btn-secondary" onClick={() => setShowDeleteModel(false)}>
                          Cancel
                        </button>
